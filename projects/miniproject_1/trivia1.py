...
def header(score, total_q, current_q):
    divider("═")
    print(
        f"{BOLD}{Y}  PYTHON QUIZ{RESET}  {DIM}│{RESET}  "
...
def grade(score, total):
    ...
    if pct >= 0.9:
        return f"{G}EXCEPTIONAL{RESET}", G
    if pct >= 0.7:
        return f"{Y}PASSING{RESET}", Y
    if pct >= 0.5:
        return f"{R}MARGINAL{RESET}", R
...
def load_questions(filepath):
    ...
    for i, q in enumerate(data):
        ...
        if not isinstance(q.get("options"), list) or len(q["options"]) < 2:
            ...
            sys.exit(1)
        if not isinstance(q.get("answer"), int) or not (
            0 <= q["answer"] < len(q["options"])
        ):
            print(
                f"{R}Error:{RESET} Question {i + 1} 'answer' must be a valid option index."
            )
            sys.exit(1)
...
def ask_question(q, q_num, total, score):
    ...
    if correct:
        ...
        print(
            f"  {R}✗ WRONG  — correct answer: {KEYS[q['answer']]}. {options[q['answer']]}{RESET}"
        )
